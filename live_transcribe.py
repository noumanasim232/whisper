...
SILENCE_THRESHOLD_MULTIPLIER = 2.0  # Multiplier for ambient noise to determine silence threshold
...
def calibrate_noise(duration=2.0):
    ...
    print(f"Calibration complete. Average RMS: {avg_rms:.5f}, Max RMS: {max_rms:.5f}, Threshold: {threshold:.5f}")
    return threshold
...
def main():
    ...
    worker_thread = threading.Thread(target=transcribe_worker, args=(model,), daemon=True)
    worker_thread.start()
